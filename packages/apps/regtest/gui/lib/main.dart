...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Regtest UI'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _initNetwork() async {
    ...
    _mgr.stream.listen((event) => setState(() => _networkState = event));
    _mgr.refresh();
  }
  ...
  _payFromClipboardGetDelay(BuildContext c) async {
    ...
    await flutterLocalNotificationsPlugin.show(0, 'Paid Invoice', res, null);
  }
  ...
  Widget _buildMainContent() {
    if (_networkState.state == NetworkState.up) {
      final n = NetworkManager().nodeList;
      if (n.isEmpty) {
